...
/**
 * 
 * Handles the event that requires the signed PDF to be saved.
 * 
 * <p>
 ...
 */
public class SaveSignedPdfHandler implements UIEventHandlerDelegate {
    ...
    public void doHandle(UIEvent e, String arg) throws CodeException {
        File selectedFile = FileChooserHome.getInstance().getFileChooser().choosePdfFileSave(
                "signed_" + PDFHome.getInstance().getPdfName());
        if (selectedFile != null) {
            new SavePdfAction(PDFHome.getInstance().getSignedPdf(), selectedFile).exec();
        }
