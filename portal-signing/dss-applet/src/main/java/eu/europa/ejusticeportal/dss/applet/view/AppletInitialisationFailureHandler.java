...
/**
 * 
 * Handles the UI event indicating that the applet has failed to initialise.
 *  
 ...
 */
public class AppletInitialisationFailureHandler implements UIEventListenerDelegate{    
    /**
     * Handle the event
    ...
    public void doHandle(Object event) {
        if (!AppletInitSemaphore.getInstance().isReady()){
            UI.setLoading(false, true, null,null);
            UIControllerHome.getInstance().getUiController().eval(UIFunction.appletInitialisationFailure);
        }
    }
