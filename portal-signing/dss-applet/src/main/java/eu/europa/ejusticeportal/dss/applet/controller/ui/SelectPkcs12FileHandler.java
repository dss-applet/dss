...
import eu.europa.ejusticeportal.dss.applet.model.action.LoadP12Action;
import eu.europa.ejusticeportal.dss.applet.model.action.RefreshCertificateList;
...
/**
 * 
 * Handles the event that requires selection of a PKCS12 certificate store.
 *  
 ...
 */
public class SelectPkcs12FileHandler implements UIEventHandlerDelegate {
    ...
    public void doHandle(UIEvent e, String arg) throws CodeException{
        File selectedFile = FileChooserHome.getInstance().getFileChooser().choosePKCS12FileOpen();
        if (selectedFile != null) {
            new LoadP12Action(selectedFile).exec();
            Event.getInstance().fire(new P12Updated(selectedFile.getAbsolutePath()));
            new RefreshCertificateList().exec();
        }
