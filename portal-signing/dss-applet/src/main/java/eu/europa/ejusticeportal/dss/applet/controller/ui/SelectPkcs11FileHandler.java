...
import eu.europa.ejusticeportal.dss.applet.model.action.LoadLibraryAction;
...
/**
 * 
 * Handles the event that requires selection of a PKCS11 library.
 *  
 ...
 */
public class SelectPkcs11FileHandler implements UIEventHandlerDelegate {
    ...
    public void doHandle(UIEvent e, String arg) throws CodeException{
        File selectedFile = FileChooserHome.getInstance().getFileChooser().choosePKCS11FileOpen();
        if (selectedFile != null) {
            new LoadLibraryAction(selectedFile).exec();            
            new RefreshCertificateList().exec();
        }
        Event.getInstance().fire(new ShowMiddlewareHelp("0",null));
