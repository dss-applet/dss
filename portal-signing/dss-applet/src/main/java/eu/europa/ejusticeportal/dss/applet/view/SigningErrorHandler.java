 ...
 */
public class SigningErrorHandler implements UIEventListenerDelegate{

    private static final DssLogger LOG = LoggingHome.getInstance().getLogger(SigningErrorHandler.class.getSimpleName());
    /**
     * Handle the event
     * @param event the event to handle
     */
    public void doHandle(Object event) {
        SigningError e = (SigningError)event;      
        LOG.info(e.getStackTrace());
        UI.updateErrorReport(e.getStackTrace());
        UI.updateUI();
        UIControllerHome.getInstance().getUiController().eval(UIFunction.showErrorReport);
    }
