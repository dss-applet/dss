...
public final class RSACipher extends CipherSpi {
    ...
    private eu.europa.ejusticeportal.dss.mscapi.Key publicKey;
    // the private key, if we were initialized using a private key
    private eu.europa.ejusticeportal.dss.mscapi.Key privateKey;
    ...
    private void init(int opmode, Key key) throws InvalidKeyException {
        ...
        if (!(key instanceof eu.europa.ejusticeportal.dss.mscapi.Key)) {
        ...
        if (key instanceof PublicKey) {
            ...
            publicKey = (eu.europa.ejusticeportal.dss.mscapi.Key)key;
            privateKey = null;
            outputSize = publicKey.length() / 8;
        } else if (key instanceof PrivateKey) {
            mode = encrypt ? MODE_SIGN : MODE_DECRYPT;
            privateKey = (eu.europa.ejusticeportal.dss.mscapi.Key)key;
            publicKey = null;
    ...
    protected int engineGetKeySize(Key key) throws InvalidKeyException {
        ...
        if (key instanceof eu.europa.ejusticeportal.dss.mscapi.Key) {
            return ((eu.europa.ejusticeportal.dss.mscapi.Key) key).length();
