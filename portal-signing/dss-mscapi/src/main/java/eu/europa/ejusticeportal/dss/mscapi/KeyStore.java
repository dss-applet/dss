...
abstract class KeyStore extends KeyStoreSpi {
	...
	/**Flag that we are interested in smart cards only*/
	private static final int MODE_SMARTCARD = 0;
	/**Flag that we are interested in certificates from other devices only*/
	private static final int MODE_OTHERS = 1;
	private final int mode;
	
    public static final class MY extends KeyStore {
        public MY() {
            super("MY",MODE_SMARTCARD);
        }
    }

    public static final class ROOT extends KeyStore {
        public ROOT() {
            super("ROOT",MODE_SMARTCARD);
        }
    }

    public static final class MYOTHER extends KeyStore {
        public MYOTHER() {
            super("MY",MODE_OTHERS);
            
        }
    }
    public static final class ROOTOTHER extends KeyStore {
        public ROOTOTHER() {
            super("ROOT",MODE_OTHERS);
        }
    }
    ...
    KeyStore(String storeName, int mode) {
    	this.mode = mode;
        ...
        throws IOException, NoSuchAlgorithmException, CertificateException
    {
        ...
        try {
            ...
            loadKeysOrCertificateChains(getName(),mode, entries);
        ...
        Collection certCollection, Collection<KeyEntry> entries)
    {
        try
        {
            ...
            //ARHS - there can be duplicate aliases, so use the hash code to distinguish them
            //Test if the same entry already exists
            for (KeyEntry e: entries){
            	if (e.getAlias().equals(alias)){
            		alias = alias + " - " + Integer.toString(certChain[0].hashCode());
            		break;
            	}
            }
     ...
     */
    private native void loadKeysOrCertificateChains(String name,int mode,
