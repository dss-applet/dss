...
public class XadesTest {
    ...
    /**
     * Test XADES detached signature validation
     * @throws Exception
     */
    @Test
    public void testSignatureValidation2() throws Exception{
        
        SignatureStatus ss = new SignatureStatus();
        InMemoryDocument doc = new InMemoryDocument(IOUtils.toByteArray(XadesTest.class.getClassLoader().getResourceAsStream("dss/xades.pdf")));
        doc.setName("xades.pdf");
        ...
        byte signed [] = IOUtils.toByteArray(XadesTest.class.getClassLoader().getResourceAsStream("dss/xades-sig.xml"));
        RefreshingTrustedListsCertificateSource.init(hc,config);
        DocumentValidationService dvs = DocumentValidationService.getInstance();
        
        dvs.validatePdf(signed, IOUtils.toByteArray(doc.openStream()), ss, config, new OnlineCRLSource(), "xades.pdf");
        assertFalse(ss.isTamperedAfterSignature());        
